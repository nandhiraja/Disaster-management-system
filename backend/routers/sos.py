...
def calc_priority(emergency_type: str, people_count: int) -> int:
    score = 0
    if emergency_type == "medical":
        score += 50
    elif emergency_type == "flood":
        score += 30
    elif emergency_type == "trapped":
        score += 40
    elif emergency_type == "elderly":
        score += 35
    if people_count >= 4:
        score += 20
    elif people_count >= 2:
...
def create_sos(data: SOSCreate):
    ...
    try:
        ...
        priority = calc_priority(data.emergency_type, data.people_count)

        conn.execute("""
            INSERT INTO sos_requests
            (sos_id, source_type, phone, lat, lon, approx_loc, people_count,
             emergency_type, status, priority_score, assigned_mission, created_at, updated_at)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)
        """, (sos_id, data.source_type, data.phone, data.lat, data.lon,
              data.approx_loc or f"{data.lat},{data.lon}", data.people_count,
              data.emergency_type, "pending", priority, None, now, now))
        conn.commit()
        return {"sos_id": sos_id, "priority_score": priority, "status": "pending", "message": "SOS received. Help is on the way!"}
    finally:
        conn.close()
